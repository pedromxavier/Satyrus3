...
class MetaSatAPI(type):
    ...
    def __new__(cls, name: str, bases: tuple, attrs: dict):
        if name == cls.base_class_name:
            new_class = cls.base_class = type.__new__(
                cls,
                name,
                bases,
                {**attrs, "subclasses": cls.subclasses, "options": cls.options},
            )
        elif name in cls.options:
            raise NameError(f"Class `{name}` is already defined.")
        elif cls.base_class is None:
            raise NotImplementedError(
                f"Base class `{cls.base_class_name}` was not implemented."
            )
        elif "_solve" not in attrs:
            raise NotImplementedError(
                f"Method _solve(self, posiform: dict) must be implemented for class {name}."
            )
        else:
            imports = []
            missing = []
    ...
    class missing:
        def __init__(self, name: str, packages: list):
            self.name = name
            self.packages = [
                package["pip"] if "pip" in package else package["import"]
        ...
        def __call__(self, *args, **kwargs):
            (
                stderr[0]
                << f"There are missing packages for this solver ({self.name}). Try running:"
            )
...
class SatAPI(metaclass=MetaSatAPI):
    """
    To define a new interface one must write, in a separate Python file (``.py``, ``.pyw``), a ``SatAPI`` subclass that implements the ``solve(self, posiform: Posiform, **params: dict) -> tuple[dict, float] | object`` method.
    ...
    def __getitem__(self, key: str):
        if key in self.subclasses:
            return self.subclasses[key](
                source_path=self.source_path, satyrus=self.satyrus, **self.kwargs
            )
        else:
            raise NotImplementedError(
        ...
        if not path.endswith((".py", "pyw")):
            ...
            if new_options:
                ...
                for i, option in enumerate(
                    sorted(new_options, key=self.options.index), 1
                ):
                    stdlog[1] << f"\t{i}. {option}"
            else:
    ...
    def __call__(self, posiform: Posiform, **params: dict) -> tuple[dict, float]:
        """
        Parameters
        ----------
        posiform : Posiform
    ...
    def solve(self):
        ...
        if not self.code:
            answer: tuple = self.solve(self.output)
            if self.complete(answer):
                x, e = answer
                ## Discard Ancillary variables
                x = {k: v for k, v in x.items() if not k.startswith(r"$")}
                return (x, e)
...
class gurobi(SatAPI):
    ...
    def _solve(self, posiform: Posiform) -> tuple[dict, float]:
        """"""
        import cvxpy as cp
        ...
        try:
            with devnull:
                P.solve(solver=cp.GUROBI)  # Silenced output
            y, e = X.value, P.value
...
class dwave(SatAPI):
    def _solve(self, posiform: Posiform, **params: dict) -> float:
        """
        Parameters
        ----------
        posiform : Posiform

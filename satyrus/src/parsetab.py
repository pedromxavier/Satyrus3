...
_lr_productions = [
  ...
  ('start -> code','start',1,'p_start','sat_parser.py',37),
  ('code -> code stmt','code',2,'p_code','sat_parser.py',42),
  ('code -> stmt','code',1,'p_code','sat_parser.py',43),
  ('stmt -> sys_config ENDL','stmt',2,'p_stmt','sat_parser.py',51),
  ('stmt -> def_const ENDL','stmt',2,'p_stmt','sat_parser.py',52),
  ('stmt -> def_array ENDL','stmt',2,'p_stmt','sat_parser.py',53),
  ('stmt -> def_restr ENDL','stmt',2,'p_stmt','sat_parser.py',54),
  ('sys_config -> SHARP NAME DOTS NUMBER','sys_config',4,'p_sys_config','sat_parser.py',59),
  ('sys_config -> SHARP NAME DOTS STRING','sys_config',4,'p_sys_config','sat_parser.py',60),
  ('def_const -> NAME ASSIGN literal','def_const',3,'p_def_const','sat_parser.py',65),
  ...
  ('shape -> shape index','shape',2,'p_shape','sat_parser.py',87),
  ('shape -> index','shape',1,'p_shape','sat_parser.py',88),
  ('index -> LBRA literal RBRA','index',3,'p_index','sat_parser.py',96),
  ('array_buffer -> LCUR array RCUR','array_buffer',3,'p_array_buffer','sat_parser.py',101),
  ('array -> array COMMA array_item','array',3,'p_array','sat_parser.py',106),
  ('array -> array_item','array',1,'p_array','sat_parser.py',107),
  ('array_item -> array_index DOTS literal','array_item',3,'p_array_item','sat_parser.py',115),
  ('array_index -> LPAR literal_seq RPAR','array_index',3,'p_array_index','sat_parser.py',120),
  ('literal_seq -> literal_seq COMMA literal','literal_seq',3,'p_literal_seq','sat_parser.py',125),
  ('literal_seq -> literal','literal_seq',1,'p_literal_seq','sat_parser.py',126),
  ('def_restr -> LPAR NAME RPAR NAME LBRA literal RBRA DOTS loops expr','def_restr',10,'p_def_restr','sat_parser.py',134),
  ('def_restr -> LPAR NAME RPAR NAME DOTS loops expr','def_restr',7,'p_def_restr','sat_parser.py',135),
  ('loops -> loops loop','loops',2,'p_loops','sat_parser.py',152),
  ('loops -> loop','loops',1,'p_loops','sat_parser.py',153),
  ('loop -> quant LCUR NAME ASSIGN domain COMMA conditions RCUR','loop',8,'p_loop','sat_parser.py',161),
  ('loop -> quant LCUR NAME ASSIGN domain RCUR','loop',6,'p_loop','sat_parser.py',162),
  ('quant -> FORALL','quant',1,'p_quant','sat_parser.py',170),
  ('quant -> EXISTS','quant',1,'p_quant','sat_parser.py',171),
  ('quant -> EXISTS_ONE','quant',1,'p_quant','sat_parser.py',172),
  ('domain -> LBRA literal DOTS literal DOTS literal RBRA','domain',7,'p_domain','sat_parser.py',177),
  ('domain -> LBRA literal DOTS literal RBRA','domain',5,'p_domain','sat_parser.py',178),
  ('conditions -> conditions COMMA condition','conditions',3,'p_conditions','sat_parser.py',186),
  ('conditions -> condition','conditions',1,'p_conditions','sat_parser.py',187),
  ('condition -> expr EQ expr','condition',3,'p_condition','sat_parser.py',195),
  ('condition -> expr GT expr','condition',3,'p_condition','sat_parser.py',196),
  ('condition -> expr LT expr','condition',3,'p_condition','sat_parser.py',197),
  ('condition -> expr GE expr','condition',3,'p_condition','sat_parser.py',198),
  ('condition -> expr LE expr','condition',3,'p_condition','sat_parser.py',199),
  ('condition -> expr NE expr','condition',3,'p_condition','sat_parser.py',200),
  ('condition -> expr','condition',1,'p_condition_expr','sat_parser.py',205),
  ('expr -> literal','expr',1,'p_expr','sat_parser.py',210),
  ('expr -> NOT expr','expr',2,'p_expr1','sat_parser.py',215),
  ('expr -> ADD expr','expr',2,'p_expr1','sat_parser.py',216),
  ('expr -> SUB expr','expr',2,'p_expr1','sat_parser.py',217),
  ('expr -> expr AND expr','expr',3,'p_expr2','sat_parser.py',222),
  ('expr -> expr OR expr','expr',3,'p_expr2','sat_parser.py',223),
  ('expr -> expr XOR expr','expr',3,'p_expr2','sat_parser.py',224),
  ('expr -> expr ADD expr','expr',3,'p_expr2','sat_parser.py',225),
  ('expr -> expr SUB expr','expr',3,'p_expr2','sat_parser.py',226),
  ('expr -> expr MUL expr','expr',3,'p_expr2','sat_parser.py',227),
  ('expr -> expr DIV expr','expr',3,'p_expr2','sat_parser.py',228),
  ('expr -> expr IMP expr','expr',3,'p_expr2','sat_parser.py',229),
  ('expr -> expr RIMP expr','expr',3,'p_expr2','sat_parser.py',230),
  ('expr -> expr IFF expr','expr',3,'p_expr2','sat_parser.py',231),
  ('expr -> expr LBRA expr RBRA','expr',4,'p_expr_index','sat_parser.py',236),
  ('expr -> LPAR expr RPAR','expr',3,'p_expr_par','sat_parser.py',241),
]
